...
import { ReactionNo, ReactionMessage } from './rxn.js';
...
import { ConnectModalHelper } from './connect-modal.js';
...
const lastMessageShownAtKey = 'AIPRM_lastMessageShownAt';
const lastDisconnectKey = 'AIPRM_lastDisconnect';
...
const queryParamAction = 'AIPRM_action';
...
const modelMultiselectOptions = {
  ...
  optionModifier: (o, op) => {
    ...
    if (modelStatusNo == ModelStatusNo.ACTIVE) {
      op.title = 'Active';
    } else if (modelStatusNo == ModelStatusNo.UNSUPPORTED) {
      op.classList.add('AIPRM__line-through');
      op.title = 'Unsupported by this platform';
    } else {
      op.classList.add('AIPRM__line-through');
      op.title = 'Deprecated';
    }
      ...
      title = sels
        .map((o) => {
          ...
          if (modelStatusNo == ModelStatusNo.ACTIVE) {
            ...
            t = t + ' (Unsupported by this platform)';
          } else {
            t = t + ' (Deprecated)';
          }
...
window.AIPRM = {
  ...
  /** @type {ConnectModalHelper} */
  ConnectModalHelper: null,
  ...
  async fetchStaticMessages() {
    try {
      ...
      for (const msg of this.StaticMessages) {
        if (msg.MessageBodyHTML) {
          msg.MessageBodyHTML = replaceLegalURLs(msg.MessageBodyHTML, this.Config, this.Client?.User?.Reseller);
          msg.MessageBodyHTML = msg.MessageBodyHTML.replace(
            /href=(["'])(https:\/\/www\.aiprm\.com\/[^"']+?)\1/g,
            (match, q, url) => `href=${q}${localizeWwwURL(url)}${q}`
          );
        }
      }
  ...
  initPerformanceTracking() {
    try {
      this.InitTraceID = crypto.randomUUID();
    } catch (error) {
      console.warn('AIPRM trace ID generation failed', error);
    } finally {
      // Fallback trace ID
      this.InitTraceID =
        this.InitTraceID ||
  ...
  /**
   * Track performance event for access type
   *
   * @param {AccessTypeNo} AccessTypeNoValue
   */
  trackPerformanceEvent(AccessTypeNoValue) {
    // Only track performance events on prompt templates dashboard, for now
    if (window.location.pathname !== '/') {
      return;
    }

    if (!this.InitTraceID) {
      console.warn('AIPRM performance tracking missing trace ID');
      return;
    }
    ...
    this.Client.usePrompt(
      ...
      AccessTypeNoValue,
      undefined,
      `${this.InitTraceID}:${Date.now()}`
    ).catch((error) =>
      console.warn('AIPRM performance tracking failed', error)
    );
  },

  /**
   * Track connect event for access type
   *
   * @param {AccessTypeNo} AccessTypeNoValue
   */
  trackConnectEvent(AccessTypeNoValue) {
    this.Client.usePrompt(AccessTypeNoValue, AccessTypeNoValue).catch((error) =>
      console.warn('[AIPRM] connect event tracking failed', error)
    );
  },

  /**
   * Track feature usage event for access type
   *
   * @param {AccessTypeNo} AccessTypeNoValue
   */
  trackFeatureUsageEvent(AccessTypeNoValue) {
    this.Client.usePrompt(AccessTypeNoValue, AccessTypeNoValue).catch((error) =>
      console.warn('[AIPRM] feature usage event tracking failed', error)
    );
  },

  /**
   * Poll checkUserStatus until the expected linked state is reached.
   *
   * @param {boolean} expectedLinked - the IsLinked state to wait for
   * @returns {Promise<void>}
   */
  async pollUntilLinkStatus(expectedLinked, maxRetries = 60) {
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      await new Promise((resolve) => setTimeout(resolve, 5000));
      await this.Client.checkUserStatus();

      if (this.Client.User?.IsLinked === expectedLinked) {
        return;
      }
    }

    throw new Error('Timed out waiting for account status update');
  },
  ...
  async init() {
    ...
    // Fetch tutorial sitemap for URL localization (must complete before localizeWwwURL is used)
    await fetchWwwURLMap(this.CacheBuster);
    updateLocalizedURLs();
    ...
    if (userOpenAI?.id) {
      this.Client.User = {
        ...
        OperatorPlanType: getOperatorPlanType(userOpenAI.id),
    ...
    this.trackPerformanceEvent(AccessTypeNo.AIPRMExtensionInit);
    ...
    await Promise.all([
      ...
      // Fetch Gizmos only for non-anon operators (anon operators cannot access Gizmos)
      !this.Client?.User?.OperatorERID?.startsWith(AnonOperatorERIDPrefix)
    ...
    // Init models config for prompt panel
    this.fetchModelsConfig();

    // Sort lists after fetching of UserTeamM
    this.Lists.sort();

    this.replaceTextDecoder();

    this.replaceFetch();

    // Initialize OAuth2 for protocol + rollout logic
    this.OAuth2 = new OAuth2(this.Client, this.Config);

    this.ConnectModalHelper = new ConnectModalHelper(
      this.Config,
      this.OAuth2,
      lastDisconnectKey,
      this.Client?.User
    ...
    if (
      this.canShowMessages() &&
      !this.isMessageShown &&
      !showOnboarding(this.CacheBuster, this.Client.User?.IsLinked) &&
      !this.showConnectAnnouncementIfEligible()
    ) {
      this.showMessages();
    }
    ...
    this.trackPerformanceEvent(AccessTypeNo.AIPRMExtensionStartDrawFirst);
    ...
    // listen for AIPRM.tokens event from AIPRM APP (legacy linking flow)
    ...
    // Ask content script for prefillPrompt (returned via AIPRM.prompt event) after initialization
    window.postMessage({
      from: 'AIPRM',
      data: { type: 'AIPRM.prefillPrompt' },
    });
  },

  // send AIPRM.getStylesheet message to content script and listen for response
  getStylesheetURL() {
    window.addEventListener('message', (event) => {
      if (
        event?.data?.from === 'AIPRM' &&
        event?.data?.type === 'AIPRM.stylesheetURL' &&
        event?.data?.url
      ) {
        this.StylesheetURL = event.data.url;

        this.monitorStylesheet();
      }
    });
  ...
  setPromptPanelHintPlaceholder(inputField) {
    ...
    this.updatePromptTextareaPlaceholder(
      inputField,
      this.Config.getPromptPanelConfig().PlaceholderText
    );
  },

  /**
   * Update the placeholder text of the prompt textarea
   * (using CSS variable or textarea placeholder attribute)
   *
   * @param {Element} inputField
   * @param {string} placeholderText
   * @returns
   */
  updatePromptTextareaPlaceholder(inputField, placeholderText) {
    const placeholderElement = inputField;

    // fallback for textarea
    if (placeholderElement.tagName === 'TEXTAREA') {
      placeholderElement.placeholder = placeholderText;

      return;
    }

    // update the placeholder text using CSS variable
    placeholderElement.style.setProperty(
      '--placeholder-text',
      `"${placeholderText.replace(/'/g, "\\'").replace(/"/g, '\\"')}"`
    );

    placeholderElement.title = placeholderText;
  },
  ...
  insertPromptPanel() {
    ...
    new PromptPanel(
      ...
      this.Config.getPromptPanelConfig().TriggerKey,
      () => this.ModelsConfig
    );
  },
  ...
  extractCurrentGizmoInformation() {
    ...
    if (GizmoCode?.[1]) {
      ...
      this.CurrentGizmo = {
        ...
        Title: title?.firstChild?.textContent || title?.textContent || '',
      };
  ...
  addLicenseWarning() {
    ...
    const config = this.Config.getUserLicenseWarningConfig();
    ...
    if (licenseWarningDismissed) {
      const now = new Date();
      if (
        Number(licenseWarningDismissed) + config.DismissedTimeout >
        now.getTime()
      ) {
        return;
      }
  ...
  async fetchConfig() {
    try {
      ...
      updateSupportedUILangs(this.Config.getSupportedUILangs());
...
`;
  ...
  // handle AIPRM.tokens event from AIPRM APP (legacy linking flow)
  async handleTokensEvent(event) {
    if (await this.isAlreadyConnected()) {
      return;
    }

    const OperatorERID = this.Client?.User?.OperatorERID || '';
    const OperatorPlanType = this.Client?.User?.OperatorPlanType || '';
    const isAnonOperator = OperatorERID.startsWith(AnonOperatorERIDPrefix);
    const isOAuth2LinkingEligible = this.OAuth2?.isLinkingEligible?.();

    // Anon operators: use OAuth2 rollout config to determine linking eligibility
    if (isAnonOperator && !isOAuth2LinkingEligible) {
      this.showNotification(
        NotificationSeverity.WARNING,
    ...
    try {
      ...
      this.showNotification(
        NotificationSeverity.ERROR,
        'Could not connect your ChatGPT account to AIPRM, please try again later.',
        false
      );
    ...
    const connectModalHelper = this.ConnectModalHelper;
    const variant = connectModalHelper.getVariant();

    if (isOAuth2LinkingEligible) {
      // show connect modal - could be hidden when starting from other place that quota.js
      connectModalHelper.showModal();
    }
    ...
    try {
      ...
      if (!this.Client.User.IsLinked) {
        if (isOAuth2LinkingEligible) {
          connectModalHelper.updateState(
            NotificationSeverity.INFO,
            variant?.ConnectingMessage ||
              'Connecting your ChatGPT account to AIPRM, please wait...'
          );
        } else {
          this.showNotification(
            NotificationSeverity.INFO,
        ...
        await this.pollUntilLinkStatus(true);
      }
    } catch (error) {
      if (isOAuth2LinkingEligible) {
        connectModalHelper.updateState(
          NotificationSeverity.ERROR,
          variant?.ErrorProfileMessage ||
    ...
    const legacyVariants = oauth2Config?.Config?.LegacyConnectVariant || {};

    const lang = getPreferredSupportedLang(Object.keys(legacyVariants));
    const legacyVariant = legacyVariants[lang] || legacyVariants['en'] || {};
    const legacyConnectParam = legacyVariant.ConnectParams || '';

    // Insert legacy ConnectLegacy logging image before redirect
    if (legacyVariant.ConnectLegacy) {
      const legacyLoggingImage = replaceEnvironmentURLs(
        legacyVariant.ConnectLegacy.replaceAll(
          '{{OperatorPlanType}}',
          OperatorPlanType
        ).replaceAll('{{OperatorERID}}', OperatorERID)
      );

      const imageContainer = document.createElement('div');
      imageContainer.style.cssText =
        'position:absolute;width:0;height:0;overflow:hidden;';
      imageContainer.innerHTML = legacyLoggingImage;
      document.body.appendChild(imageContainer);
    }

    // Redirect to connect-success page — AIPRM reinitializes on return
    window.location.href = appendLangParam(
      `${OAuth2Config.ConnectSuccessURL}?type=chatgpt&success=${ReactionNo.RXN_AIPRM_CHATGPT_ACCOUNT_CONNECTED}${legacyConnectParam ? `&${legacyConnectParam}` : ''}&connectOperatorERID=${encodeURIComponent(OperatorERID)}&planType=${encodeURIComponent(OperatorPlanType)}`
    );
  },
  ...
  async fetchPromptFromDeepLink() {
    ...
    this.trackFeatureUsageEvent(AccessTypeNo.UsePromptFromParam);
  ...
  async initSearchTermFromParam() {
    ...
    if (paramSearchTerm) {
      if (window.location.href.match(this.Config.getEndpointGizmos())) {
        ...
        this.trackFeatureUsageEvent(AccessTypeNo.PromptSearchFromParam);
        this.PromptSearch = paramSearchTerm;
      }
    }
  },

  async handleActionFromParam() {
    const params = new URLSearchParams(window.location.search);
    const action = params.get(queryParamAction);

    if (!action) {
      return;
    }

    // Clean the URL parameter to prevent re-triggering on navigation
    params.delete(queryParamAction);
    const cleanURL = params.toString()
      ? `${window.location.pathname}?${params.toString()}`
      : window.location.pathname;
    window.history.replaceState({}, '', cleanURL);

    if (action === 'connect' && !this.Client.User?.IsLinked) {
      if (this.shouldShowOAuth2ConnectCTA()) {
        await this.oauth2Login();
      } else {
        this.connectAccount();
      }
    } else if (action === 'disconnect' && this.Client.User?.IsLinked) {
   ...
   */
  showMessages() {
    if (this.isMessageShown || !this.canShowMessages()) {
      return;
    }

    const remainingMs = this.getMessageDelayRemainingMs();

    if (remainingMs > 0) {
      this.scheduleDelayedMessageShow(remainingMs);
      return;
    }

    console.debug('[AIPRM MessageDelay] showing message');
    ...
    this.isMessageShown = showMessage(
      ...
      this.Config,
      this.Client?.User?.Reseller
    );
  },

  /** Returns ms remaining in the message delay cooldown, or 0 if no delay is active. */
  getMessageDelayRemainingMs() {
    const delayMinutes = this.Config?.getMessageDelayMinutes() ?? 0;
    ...
    const lastShownAt = Number(
      localStorage.getItem(lastMessageShownAtKey) || 0
    );
    ...
    return Math.max(0, lastShownAt + delayMinutes * 60000 - Date.now());
  },

  /**
   * Schedule the next message show using a precomputed cooldown when available
   * to avoid races between separate delay checks.
   *
   * @param {number} [remainingMs]
   */
  scheduleDelayedMessageShow(remainingMs = this.getMessageDelayRemainingMs()) {
    clearTimeout(this.messageDelayTimer);
    ...
    console.debug(
      `[AIPRM MessageDelay] scheduled next message in ${Math.round(remainingMs / 1000)}s`
    );

    this.messageDelayTimer = setTimeout(() => this.showMessages(), remainingMs);
  },

  setLastMessageShownAt() {
    try {
      localStorage.setItem(lastMessageShownAtKey, Date.now());
    } catch (e) {
      console.error(
        '[AIPRM] Could not set last message shown time in local storage',
        e
      );
    }

    this.scheduleDelayedMessageShow();
  },
  ...
  async confirmMessage(MessageID) {
    ...
    try {
      // Resolve confirm logging image from remote config
      let legacyLoggingImage = '';
      const imageConfig = this.Config?.getMessageConfirmImageConfig?.();
      if (imageConfig) {
        const lang = getPreferredSupportedLang(Object.keys(imageConfig));
        const image = imageConfig[lang] || imageConfig['en'] || '';

        if (image) {
          legacyLoggingImage = `<div style="position:absolute;width:0;height:0;overflow:hidden;">${replaceEnvironmentURLs(
            image
              .replaceAll('{{MessageID}}', MessageID)
              .replaceAll(
                '{{OperatorPlanType}}',
                this.Client?.User?.OperatorPlanType || ''
              )
              .replaceAll(
                '{{OperatorERID}}',
      ...
      this.showNotification(
        NotificationSeverity.SUCCESS,
        'Thanks for the confirmation!' + legacyLoggingImage
      );
    } catch (e) {
      console.error('[AIPRM] confirmMessage post-processing failed', e);
    } finally {
      this.isMessageShown = false;
      this.setLastMessageShownAt();
    }
  ...
  // Replace text decoder to modify conversation chat messages to not show prompt templates
  replaceTextDecoder() {
    // Skip if modification of conversation chat messages is disabled
    if (
      !this.Config.getPromptTemplatesConfig().EnableTextDecoderMessagesFilter
    ) {
      return;
    }

    // Skip selected content types from filtering
    const skipContentTypes =
      this.Config.getPromptTemplatesConfig()
        ?.TextDecoderMessagesSkipContentTypes ?? [];

    // Store original TextDecoder
    const OriginalTextDecoder = window.TextDecoder;

    // Create proxy for TextDecoder
    window.TextDecoder = function (label = 'utf-8', options = {}) {
      const decoder = new OriginalTextDecoder(label, options);
      const originalDecode = decoder.decode;

      decoder.decode = function (input, options = {}) {
        const text = originalDecode.call(decoder, input, options);

        // If starts with { and it's possibly user message (try to parse JSON)
        if (!text.startsWith('{') || !text.includes('"user"')) {
          return text;
        }
        ...
        try {
          ...
          const msg =
            message?.message || message?.v?.message || message?.input_message;

          const contentType = msg?.content?.content_type;

          // Skip selected content types (eg: used for custom instructions)
          if (
            typeof contentType === 'string' &&
            skipContentTypes.includes(contentType)
          ) {
            return text;
          }

          // Only the first message from the user
    ...
    window.TextDecoder.prototype = OriginalTextDecoder.prototype;
    window.TextDecoder.prototype.constructor = window.TextDecoder;
  },
  ...
  replaceFetch() {
    window.fetch = async (...t) => {
      ...
      // Cannot proceed without request URL to match known endpoints
      if (!t?.[0]?.match) {
        return this.fetch(...t);
      }
      ...
      // Modify the conversation chat messages request for the current conversation to hide prompt templates
      if (config.EnableConversationMessagesModification) {
        const conversationID = t?.[0].match(
          config.EndpointConversationChatMessages
        )?.[2];
        ...
        if (
          ...
          // console.log(
          //   'replaceFetch: current conversation - modifying messages',
          //   conversationID
          // );

          return this.modifyConversationMessages(async () => this.fetch(...t));
        }
      }
      ...
      // Index of the user-authored message in the request body
      ...
      try {
        ...
        if (this.Config.isPromptSendQuotaEnabled()) {
          ...
          // Reset the continue action selection
          this.resetContinueActionSelect();
        ...
        if (this.Config?.isShowMessagesOnPromptSubmitEnabled()) {
          this.showMessages();
        }

        // Regenerate response has no body.messages
        if (!body.messages?.[0]) {
          return this.fetch(...t);
        }

        // Find the first user-authored message in the request body
        ...
        targetMessageIndex =
          firstUserMessageIndex >= 0 && messages[firstUserMessageIndex]
            ? firstUserMessageIndex
            : 0;

        // Get the index of the message part in the request body
        messagePartIndex = this.findMessagePartIndex(
          body.messages[targetMessageIndex]
        );

        // Get the prompt from the request body
        prompt =
          body.messages[targetMessageIndex].content.parts[messagePartIndex];
      ...
      try {
        ...
        useAuxIndexLookup =
          this.Client.UserQuota.hasCustomIndexesFeatureEnabled() &&
          body.messages[targetMessageIndex].content.parts[
            messagePartIndex
          ].match(AuxIndexLookupDefinition);
  ...
  addWatermark() {
    ...
    // watermark is hidden by user settings
    if (this.getHideWatermark()) {
      return;
    }

    this.insertInlineWatermarkToggle();
  ...
  insertInlineWatermarkToggle() {
    const selectorConfig = this.Config.getSelectorConfig();
    if (!selectorConfig) {
      return;
    }

    // add watermark toggle to all chat responses
    document
      .querySelectorAll(selectorConfig.ResponseButtonsBlock)
      .forEach((buttonsDiv) => {
        if (!buttonsDiv?.children?.length) {
          return;
        }

        let watermarkToggle = buttonsDiv.querySelector(
          'span.AIPRM__watermarkToggle'
        );

        // if not already added, create a new watermark toggle button
        if (!watermarkToggle) {
          watermarkToggle = document.createElement('span');
          watermarkToggle.className =
            'AIPRM__watermarkToggle AIPRM__rounded-lg hover:AIPRM__bg-gray-100 AIPRM__text-gray-700 dark:AIPRM__text-gray-50 dark:hover:AIPRM__bg-gray-800 AIPRM__cursor-pointer';
          watermarkToggle.title = 'Hide AIPRM watermark';
          watermarkToggle.addEventListener(
            'click',
            this.toggleWatermarkInline.bind(this)
          );
          watermarkToggle.innerHTML = `<span class="AIPRM__flex AIPRM__h-8 AIPRM__w-8 AIPRM__items-center AIPRM__justify-center">${svg(
            'EyeSlash'
          )}</span>`;

          // Insert before resizing regenerate response button (if available)
          if (buttonsDiv.children.length >= 2) {
            buttonsDiv.insertBefore(
              watermarkToggle,
              buttonsDiv.children[buttonsDiv.children.length - 2]
            );
          } else {
            buttonsDiv.insertBefore(watermarkToggle, buttonsDiv.lastChild);
          }
        }
      });
  },

  toggleWatermarkInline() {
    this.trackFeatureUsageEvent(AccessTypeNo.ToggleWatermark);
    ...
    const currentValue = localStorage.getItem(hideWatermarkKey) || '';

    // update local storage
    try {
      localStorage.setItem(hideWatermarkKey, currentValue ? '' : 'true');
    } catch (error) {
      this.showNotification(
        NotificationSeverity.ERROR,
    ...
    Object.keys(watermarkConfig.Selectors).forEach((watermarkClass) => {
      document
        .querySelectorAll(`${watermarkConfig.Selectors[watermarkClass]}`)
        .forEach((element) => {
          element.classList.toggle(watermarkClass);
        });
    });

    // remove all watermark toggle buttons
    document
      .querySelectorAll('span.AIPRM__watermarkToggle')
      ?.forEach((element) => {
        element.remove();
      });
  },
  ...
  async handleElementAdded(e) {
    ...
    // #region Ignore added elements
    // Ignore changes in the prompt textarea & Canvas to prevent performance issues when pasting large texts
    if (
      e.parentElement?.id &&
      selectorConfig.IgnoreByParentID?.includes(e.parentElement?.id)
    ) {
      return;
    }
    ...
    if (
      ...
      selectorConfig.IgnoreByParentClassPrefix?.some((prefix) =>
        e.parentElement?.className?.startsWith?.(prefix)
      )
    ) {
      return;
    }
    ...
    if (
      e.className &&
      selectorConfig.IgnoreByElementClassPrefix?.some((prefix) =>
        e.className?.startsWith?.(prefix)
      )
    ) {
      return;
    }

    if (selectorConfig.IgnoreByTag?.includes(e.tagName?.toLowerCase?.())) {
      return;
    }

    if (
      selectorConfig.IgnoreElementsWithoutIdentifiers &&
      !e.className &&
      !e.id &&
      !e.parentElement?.className
    ) {
      // Ignore elements without class name, ID or parent element class name
      return;
    }
    // #endregion
    ...
    if (
      ...
      (e.className &&
        selectorConfig.ElementAddedSidebarClassNames.includes(e.className))
    ) {
      this.setupSidebar();
      return;
    }
    ...
    if (
      e.className &&
      selectorConfig.GizmosContentContainers.includes(e.className) &&
    ...
    // Unselect prompt when dictation is used (new chat started)
    if (
      document.querySelector(selectorConfig.ComposerDictationOverlay) &&
      this.SelectedPromptTemplate
    ) {
      this.unselectPromptTemplate();
    }
  ...
  // Unselect prompt template and remove variable values from URL
  unselectPromptTemplate() {
    this.PromptVariables = {};
    this.SelectedPromptTemplate = null;

    try {
      // Remove Prompt ID and variable values from URL
      const url = new URL(window.location.href);

      const queryParams = [...url.searchParams.keys()].filter(
        (key) =>
          key.startsWith(queryParamVariable) || key === queryParamPromptID
      );

      // Only update history if there are query params to remove
      ...
      console.error('Error in unselectPromptTemplate:', error);
    }
  },
  ...
  // Monitor if the AIPRM stylesheet is still present in head and add it again if needed
  monitorStylesheet() {
    ...
    if (
      this.Config.getPromptTemplatesConfig().EnableDuplicateStylesheetMonitor
    ) {
      try {
        const existingStylesheets = [
          ...document.querySelectorAll('head link[rel="stylesheet"]'),
        ];

        const uniqueStylesheetHrefs = new Set();

        // Process existing stylesheets
        existingStylesheets?.forEach((stylesheet) => {
          if (stylesheet.href && !uniqueStylesheetHrefs.has(stylesheet.href)) {
            uniqueStylesheetHrefs.add(stylesheet.href);
            return;
          }
        ...
        console.error('Error monitoring duplicate stylesheets:', error);
      }
    }

    // Return if the stylesheet is already present
    if (document.querySelector('link[href="' + this.StylesheetURL + '"]')) {
      return;
    }
  ...
  insertPromptTemplatesSidebar() {
    ...
    sidebarIcon.addEventListener('click', () => {
      ...
      !sidebarOpen
        ? sidebarIcon.classList.add('AIPRM__hidden')
        : sidebarIcon.classList.remove('AIPRM__hidden');
      ...
      sidebar.classList.toggle('AIPRM__opacity-100');
      sidebar.classList.toggle('AIPRM__opacity-0');
      sidebar.classList.toggle('AIPRM__fixed');
      sidebar.classList.toggle('AIPRM__absolute');
    ...
    sidebar.className =
      '2xl:AIPRM__w-7/12 lg:AIPRM__w-3/4 md:AIPRM__w-11/12 AIPRM__w-full AIPRM__h-full AIPRM__bg-white AIPRM__text-gray-700 AIPRM__shadow-lg AIPRM__fixed AIPRM__right-0 AIPRM__overflow-auto AIPRM__z-20 dark:AIPRM__bg-gray-900 dark:AIPRM__text-white AIPRM__ease-in-out AIPRM__duration-300 AIPRM__translate-x-full dark:AIPRM__shadow-gray-600/50 AIPRM__opacity-0';

    sidebar.innerHTML = /*html*/ `
          <div class="AIPRM__relative" id="AIPRM__sidebar-container">
            <div title="Close AIPRM sidebar" class="AIPRM__p-2 AIPRM__items-center AIPRM__transition-colors AIPRM__duration-200 AIPRM__cursor-pointer AIPRM__text-sm AIPRM__rounded-md AIPRM__border AIPRM__bg-white dark:AIPRM__bg-gray-900 AIPRM__border-black/10 dark:AIPRM__border-white/20 hover:AIPRM__bg-gray-50 dark:hover:AIPRM__bg-gray-850 AIPRM__absolute AIPRM__top-0 AIPRM__left-4 AIPRM__z-30" onclick="document.getElementById('AIPRM__sidebar-icon').click()">
  ...
  insertSavePromptAsTemplateButton() {
    ...
    saveButton.className =
      'save-prompt-button AIPRM__rounded-md hover:AIPRM__bg-gray-100 AIPRM__text-gray-700 dark:AIPRM__text-gray-50 dark:hover:AIPRM__bg-gray-800 disabled:dark:hover:AIPRM__text-gray-400 md:AIPRM__invisible md:group-hover:AIPRM__visible md:group-hover:visible';
    saveButton.title = 'Save prompt as template';
    saveButton.addEventListener('click', this.createNewPrompt.bind(this));
    saveButton.innerHTML = `<span class="AIPRM__flex AIPRM__h-8 AIPRM__w-8 AIPRM__items-center AIPRM__justify-center">${svg(
      'Save'
    )}</span>`;
  ...
  validatePrompt(prompt, currentCreatePromptMode) {
    ...
    if (validateVariablesResult.EnumMaxSizeError) {
      ...
      errorMessage += `<br><a class="AIPRM__underline" href="${localizeWwwURL('https://www.aiprm.com/tutorials/create-prompts/how-to-use-variables-in-prompts/#title-how-to-use-variables-in-prompts-0')}" target="_blank" rel="noopener noreferrer">Learn more about prompt variables</a>.`;
    ...
    // require AuthorName if prompt is public
    if (
      prompt.PromptTypeNo === PromptTypeNo.PUBLIC &&
      !prompt.AuthorName.trim()
    ) {
      errors.push(
        'Please identify with Author Name to publish a prompt template as public.'
      );
    }
    ...
    if (missingPlaceholders.length > 0) {
      errors.push(
        currentCreatePromptMode === CreatePromptMode.BASIC
          ...
          Learn more <a class="AIPRM__underline" href="${localizeWwwURL('https://www.aiprm.com/tutorials/create-prompts/aiprm-prompt-template-guidelines/')}" target="_blank" rel="noopener noreferrer">here</a>.
        `
      );
    }
      ...
      if (
        ...
        if (ownPrivateLists.length == 0) {
          ...
          if (isInOwnPrivateList) {
            ...
            if (
              !this.isAdminMode() &&
              !this.Client.UserQuota.canAddToCustomList(list)
            ) {
              return;
            }
  ...
  showNotification(
          ...
          <div>
            ...
            ${
              severity === NotificationSeverity.ERROR
                ? /*html*/ `<br/><p class="AIPRM__max-w-md AIPRM__font-semibold" style="overflow-wrap: anywhere;">If this issue persists, please check <a href="https://help.aiprm.com/" target="_blank" class="AIPRM__underline">our Help Center</a> for troubleshooting and <a href="https://status.aiprm.com/" target="_blank" class="AIPRM__underline">status.aiprm.com</a> for updates.</p>`
                : ''
            }
          </div>
          <button>${svg('Cross')}</button>
    ...
    getNotificationContainer().prepend(notificationElement);
  },

  hideModal,

  shouldShowOAuth2ConnectCTA() {
    return this.OAuth2.isLinkingEligible();
  },

  /**
   * Whether messages can be shown on the current page based on path.
   * Returns false when the current pathname matches SuppressMessagesPathPattern.
   * Defensive: returns true (allow) if Config or the pattern is missing, empty, or invalid.
   *
   * Note: time-based message delay is checked separately in showMessages().
   *
   * @returns {boolean}
   */
  canShowMessages() {
    try {
      ...
      const pattern = this.Config.getSuppressMessagesPathPattern();
      if (!pattern) {
        return true;
      }
      return !new RegExp(pattern).test(window.location.pathname);
    } catch (e) {
      console.warn('[AIPRM] invalid SuppressMessagesPathPattern', e);
      return true;
    }
  },

  /** @returns {boolean} */
  showConnectAnnouncementIfEligible() {
    const OperatorERID = this.Client?.User?.OperatorERID || '';
    const isAnonOperator = OperatorERID.startsWith(AnonOperatorERIDPrefix);

    // Anon operators: use OAuth2 rollout config to determine linking eligibility
    if (isAnonOperator && !this.OAuth2?.isLinkingEligible?.()) {
      return false;
    }

    // Known operators: use legacy PatternOperatorERID match (not OAuth2 rollout)
    if (
      !isAnonOperator &&
      !OperatorERID.match(this.Config.getPatternOperatorERID())
    ) {
      return false;
    }
    ...
    return (
      this.Client.UserQuota.connectAccountAnnouncement(
        this.shouldShowOAuth2ConnectCTA(),
        this.ConnectModalHelper
      ) === true
    );
  },
  ...
  async showSavePromptModal(e, promptPlugins) {
    ...
    if (
      ...
      // Get prompt from the closest prompt container based on the clicked element
      const prompt = e.target
        .closest(selectorConfig.SavePromptAsTemplateContainer)
        ?.querySelector(selectorConfig.SavePromptAsTemplatePromptText);
                    ...
                    <div class="AIPRM__block">
                      <div class="AIPRM__flex AIPRM__justify-between AIPRM__mt-4">
                        ...
                        <div class="AIPRM__w-full"><label>Author URL</label>
                          <input name="AuthorURL" type="url" title="Author URL associated with this prompt template"
  ...
  buildBasicPrompt() {
    ...
    // Otherwise set PromptTypeNo to PRIVATE
    else {
      form.elements['PromptTypeNo'].value = PromptTypeNo.PRIVATE;
    }
  ...
  addExportButton() {
    ...
    version.innerHTML = /*html*/ `${svg`Rocket`} ${NavTutorialsLabel}`;
    version.href = NavTutorialsURL;
    ...
    // Create the "AIPRM Example Use Cases" element
    const forum = document.createElement('a');
    forum.className = css('VersionInfo');
    forum.innerHTML = `${svg('Topic')} ${NavExampleUseCasesLabel}`;
    forum.href = NavExampleUseCasesURL;
    forum.target = '_blank';
  ...
  async insertPromptTemplatesSection(isFirstDraw = false) {
    ...
    const paginationContainer = /*html*/ `
    <div class="AIPRM__flex AIPRM__flex-1 AIPRM__gap-3.5 AIPRM__justify-between AIPRM__items-center AIPRM__flex-col @sm/templates-wrapper:AIPRM__flex-row AIPRM__mt-6">
      <div class="AIPRM__text-left" style="margin-top: -1rem;">
        ...
        <select title="Prompts per Page" class="AIPRM__bg-gray-100 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded AIPRM__block AIPRM__w-full dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-gray-200 focus:AIPRM__ring-0 dark:hover:AIPRM__bg-gray-800 pageSizeSelect">
          ${pageSizeOptions
            .map(
              (pageSize) => /*html*/ `
                <option value="${pageSize}" ${
      ...
      <div class="${css`paginationButtonGroup`}">
        <button title="Previous page" onclick="AIPRM.prevPromptTemplatesPage()" class="${css`paginationButton`}" style="border-radius: 6px 0 0 6px">Prev</button>
        <button title="Next page" onclick="AIPRM.nextPromptTemplatesPage()" class="${css`paginationButton`} AIPRM__border-0 AIPRM__border-l AIPRM__border-gray-500" style="border-radius: 0 6px 6px 0">Next</button>
      </div>
    </div>
  `;
    ...
    <div class="${css`column`} AIPRM__relative" style="min-width: 0">
      ...
      ${
        isSidebarView
          ? /*html*/ `
          <div class="@lg/templates-wrapper:AIPRM__absolute AIPRM__top-0 AIPRM__right-0 AIPRM__text-right">
            ...
            <div class="AIPRM__mb-2">
              ${
                this.Client.AppUser?.IsLinked
                  ...
                  : this.shouldShowOAuth2ConnectCTA()
                    ? /*html*/ `
                      <a class="AIPRM__underline" href="" onclick="event.preventDefault(); AIPRM.oauth2Login()">Login</a>
                    `
      ...
      <ul class="AIPRM__flex AIPRM__flex-col AIPRM__gap-3.5 AIPRM__mb-4">
        <div class="@md/templates-wrapper:AIPRM__hidden AIPRM__text-left AIPRM__text-sm">
          <label class="AIPRM__block AIPRM__text-sm AIPRM__font-medium">Prompt List</label>

          <a href="#" title="View / hide lists" 
            onclick="AIPRM.toggleListViewVisibility()" 
            class="AIPRM__rounded AIPRM__flex AIPRM__justify-center AIPRM__items-center AIPRM__p-2 AIPRM__font-medium AIPRM__text-gray-800 AIPRM__bg-gray-100 hover:AIPRM__bg-gray-200 dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-600 dark:AIPRM__text-gray-400 dark:hover:AIPRM__text-white dark:hover:AIPRM__bg-gray-800">
            <div class="AIPRM__w-full AIPRM__text-center AIPRM__flex AIPRM__justify-center AIPRM__items-center">${this.createCurrentListTitle()}</div>

            <div>
              <span id="listsViewToggleOpen">${svg('ChevronDown')}</span>
              <span id="listsViewToggleClose" class="AIPRM__hidden">${svg(
                'ChevronUp'
              )}</span>
            </div>
          </a>
        ...
        <ul id="listsView" class="AIPRM__border-b AIPRM__border-gray-200 dark:AIPRM__border-gray-700 dark:AIPRM__text-gray-400 AIPRM__hidden @md/templates-wrapper:AIPRM__flex AIPRM__flex-wrap AIPRM__font-medium AIPRM__text-center AIPRM__text-gray-500 AIPRM__text-sm AIPRM__whitespace-nowrap">
          <li class="AIPRM__flex-1 AIPRM__mr-2">
            <a href="#" id="favoritePromptsTab" title="Your &quot;Favorites&quot; List" 
              onclick="AIPRM.trackFeatureUsageEvent(${AccessTypeNo.ViewFavoritesTab}); ${
                favoritesList
          ...
          <li class="AIPRM__flex-1 AIPRM__mr-2">
            <a href="#" id="AIPRMVerifiedPromptsTab" title="&quot;AIPRM Verified&quot; Prompts List" 
              onclick="AIPRM.trackFeatureUsageEvent(${AccessTypeNo.ViewAIPRMTab}); AIPRM.Client.UserQuota.canUseAIPRMVerifiedList() ? AIPRM.changePromptTemplatesType('${
          ...
          <li class="AIPRM__flex-1 AIPRM__mr-2">
            <a href="#" id="publicPromptsTab" title="Public Prompts"  
            onclick="AIPRM.trackFeatureUsageEvent(${AccessTypeNo.ViewPublicTab}); AIPRM.changePromptTemplatesType('${
          ...
          <li class="AIPRM__flex-1 AIPRM__mr-2">
            ...
            onclick="AIPRM.trackFeatureUsageEvent(${AccessTypeNo.ViewOwnTab}); AIPRM.changePromptTemplatesType('${
                  ...
                  .map(
                    ...
                    <li class="AIPRM__flex-1 AIPRM__mr-2">
                      <a href="#" title="${list.createTitle(
                        this.Client
                      )}" onclick="AIPRM.trackFeatureUsageEvent(${list.ForTeamID ? AccessTypeNo.ViewTeamListTab : AccessTypeNo.ViewCustomListTab}); AIPRM.changePromptTemplatesType('${
                        PromptTemplatesType.CUSTOM_LIST
                      }', '${list.ID}')"
                      class="${
          ...
          <li class="AIPRM__flex-1">
            ...
            onclick="AIPRM.trackFeatureUsageEvent(${AccessTypeNo.ViewHiddenTab}); ${
              hiddenList
        ...
        <div class="AIPRM__grid AIPRM__grid-cols-2 @2xl/templates-wrapper:AIPRM__flex AIPRM__flex-row AIPRM__gap-3 AIPRM__items-end AIPRM__justify-between AIPRM__w-full @md/templates-wrapper:last:AIPRM__mb-6 AIPRM__pt-2 AIPRM__stretch AIPRM__text-left AIPRM__text-sm">
          <div>
            <label for="topicSelect" class="AIPRM__block AIPRM__text-sm AIPRM__font-medium">Topic</label>
        
            <select id="topicSelect" title="Filter by Topic" class="AIPRM__bg-gray-100 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded AIPRM__block AIPRM__w-full dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-gray-200 focus:AIPRM__ring-0 dark:hover:AIPRM__bg-gray-800">
          ...
          <div>
            ...
            <select id="activitySelect" title="Filter by Activity" class="AIPRM__bg-gray-100 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded AIPRM__block AIPRM__w-full dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-gray-200 focus:AIPRM__ring-0 dark:hover:AIPRM__bg-gray-800">
          ...
          <div>
            ...
            <select id="sortBySelect" title="Sort Prompts" class="AIPRM__bg-gray-100 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded AIPRM__block AIPRM__w-full dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-gray-200 focus:AIPRM__ring-0 dark:hover:AIPRM__bg-gray-800">
              ${Object.keys(SortModeNo)
                .map(
                  (sortMode) => /*html*/ `
                  <option value="${SortModeNo[sortMode]}" ${
          ...
          <div>
            ...
            <select id="modelSelect" title="Filter by Model" class="AIPRM__bg-gray-100 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded AIPRM__block AIPRM__w-full dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-gray-200 focus:AIPRM__ring-0 dark:hover:AIPRM__bg-gray-800">
          ...
          <div class="AIPRM__whitespace-nowrap AIPRM__flex">
            <button title="Create New Prompt" 
              ...
              class="AIPRM__rounded AIPRM__justify-center AIPRM__items-center AIPRM__hidden @2xl/templates-wrapper:AIPRM__inline-block AIPRM__mr-1 AIPRM__p-2 AIPRM__px-2.5 AIPRM__font-medium AIPRM__text-gray-800 AIPRM__bg-gray-100 hover:AIPRM__bg-gray-200 dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-600 dark:AIPRM__text-gray-400 dark:hover:AIPRM__text-white dark:hover:AIPRM__bg-gray-800">
              ${svg('Plus')}
            </button>
            <input id="promptSearchInput" title="Search Prompts" type="search" class="AIPRM__bg-gray-100 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded AIPRM__inline-block AIPRM__w-full dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-gray-200 focus:AIPRM__ring-0 @2xl/templates-wrapper:AIPRM__w-[260px] dark:hover:AIPRM__bg-gray-800" placeholder="Search" 
            value="${sanitizeInput(
              this.PromptSearch
            )}" onfocus="this.value = this.value">
          </div>

          <div class="@2xl/templates-wrapper:AIPRM__hidden">
            <button title="Create New Prompt" 
              onclick="event.preventDefault(); AIPRM.createNewPrompt()" 
        ...
        <ul class="${css`ul`} AIPRM__grid AIPRM__grid-cols-1 @3xl/templates-wrapper:AIPRM__grid-cols-2 ${
          !isSidebarView ? '@[1536px]/templates-wrapper:AIPRM__grid-cols-4' : ''
        }">
          ${currentTemplates
            .map((template) =>
            ...
            <button onclick="AIPRM.selectPromptTemplateByIndex(${
              ...
              ${
                !template.OwnPrompt && !isCustomListView && !isFavoritesListView
                  ? /*html*/ `
                    <div class="AIPRM__absolute AIPRM__top-0 AIPRM__left-0 AIPRM__flex AIPRM__text-gray-400 @lg/templates-wrapper:AIPRM__invisible group-hover:AIPRM__visible">
                      <a title="${
              ...
              <div class="AIPRM__flex AIPRM__items-start AIPRM__w-full AIPRM__justify-between">
                ...
                <div class="AIPRM__flex AIPRM__gap-4 AIPRM__justify-center @lg/templates-wrapper:AIPRM__gap-1 @lg/templates-wrapper:AIPRM__pl-2 AIPRM__mt-1 AIPRM__right-2 AIPRM__text-gray-400 @lg/templates-wrapper:AIPRM__invisible group-hover:AIPRM__visible">
              ...
              <div class="AIPRM__text-gray-500 AIPRM__text-xs AIPRM__flex AIPRM__pb-1 AIPRM__max-w-full">
                ...
                ${
                  template.AuthorName
                    ? /* html */ `· <span class="AIPRM__mx-1 AIPRM__overflow-hidden AIPRM__text-ellipsis AIPRM__whitespace-nowrap"
                ...
                )}" class="AIPRM__mx-1 AIPRM__whitespace-nowrap">${formatAgo(
                  template.RevisionTime
                )}</span>
                ...
                ${
                  template.ForkedFromPromptID
                    ? /*html*/ `· 
                    <a class="AIPRM__mx-1 AIPRM__underline AIPRM__whitespace-nowrap" 
   ...
   `;
    ...
    wrapper.className = `AIPRM__mt-6 md:AIPRM__flex AIPRM__items-start AIPRM__text-center AIPRM__gap-2.5 AIPRM__max-w-full AIPRM__m-auto sm:AIPRM__mx-4 AIPRM__text-sm AIPRM__@container/templates-wrapper ${
    ...
    // Track the first draw performance event
    if (isFirstDraw) {
      this.trackPerformanceEvent(AccessTypeNo.AIPRMExtensionFinishDrawFirst);
    }
  },
  ...
  toggleListViewVisibility() {
    ...
    if (listsView) {
      listsView.classList.toggle('AIPRM__hidden');
    }

    const listsViewToggleOpen = document.getElementById('listsViewToggleOpen');
    const listsViewToggleClose = document.getElementById(
      'listsViewToggleClose'
    );
    if (listsViewToggleOpen && listsViewToggleClose) {
      listsViewToggleOpen.classList.toggle('AIPRM__hidden');
      listsViewToggleClose.classList.toggle('AIPRM__hidden');
    }
  },

  createCurrentListTitle() {
    const favoritesList = this.Lists.getFavorites();
    const hiddenList = this.Lists.getHidden();
    const AIPRMVerifiedList = this.Lists.getAIPRMVerified();
    const selectedList = this.Lists.withID(this.PromptTemplatesList);
    ...
    if (
      ...
      return `${svg('StarSolidLarge')} &nbsp; Favorites`;
    } else if (
      AIPRMVerifiedList &&
      this.PromptTemplatesType === PromptTemplatesType.CUSTOM_LIST &&
      this.PromptTemplatesList === AIPRMVerifiedList?.ID
    ) {
      return `${svg('CheckBadgeSolidLarge')} &nbsp; AIPRM`;
    } else if (this.PromptTemplatesType === PromptTemplatesType.PUBLIC) {
      return 'Public';
    } else if (this.PromptTemplatesType === PromptTemplatesType.OWN) {
      return 'Own';
    } else if (
      ...
      return `${svg('EyeSlash')} &nbsp; Hidden`;
    } else if (selectedList) {
      if (selectedList.ListTypeNo === ListTypeNo.TEAM_CUSTOM) {
        if (selectedList.HasWriteAccessForTeamMember(this.Client.UserTeamM)) {
          return `${svg('TeamListSolid')} &nbsp; ${sanitizeInput(
            selectedList.Comment
          )}`;
        } else {
          return `${svg('TeamList')} &nbsp; ${sanitizeInput(
            selectedList.Comment
          )}`;
        }
      } else {
        return sanitizeInput(selectedList.Comment);
      }
    } else {
      return '';
    }
  ...
  useGizmoStarterPrompt(starterPromptID) {
    ...
    textarea.tagName === 'TEXTAREA'
      ...
      : (textarea.innerText = starterPrompt.Prompt);
          ...
          <ul title="" class="AIPRM__dropdown-menu AIPRM__hidden AIPRM__z-50 AIPRM__border AIPRM__bg-white AIPRM__absolute AIPRM__text-left AIPRM__cursor-pointer AIPRM__rounded-xl AIPRM__border-gray-100 dark:AIPRM__bg-gray-900 dark:AIPRM__border-gray-800 dark:AIPRM__shadow-xs dark:AIPRM__text-gray-100 AIPRM__shadow-xxs AIPRM__py-2">
            ...
            ${
              list.ListTypeNo === ListTypeNo.TEAM_CUSTOM
                ...
                <li class="AIPRM__flex AIPRM__items-center AIPRM__py-2 AIPRM__px-4 hover:!AIPRM__bg-gray-50 dark:hover:!AIPRM__bg-gray-700 AIPRM__text-gray-800 dark:AIPRM__text-gray-100" onclick="event.stopPropagation();window.open('${appendLangParam(`${AppTeamBaseURL}/${list.ForTeamID}`)}')">${svg('TeamPrompt')}&nbsp;Manage Team</li>`
                  : /* html */ `
                <li class="AIPRM__flex AIPRM__items-center AIPRM__py-2 AIPRM__px-4 hover:!AIPRM__bg-gray-50 dark:hover:!AIPRM__bg-gray-700 AIPRM__text-gray-800 dark:AIPRM__text-gray-100" onclick="event.stopPropagation();window.open('${appendLangParam(`${AppTeamBaseURL}/${list.ForTeamID}`)}')">${svg('TeamPrompt')}&nbsp;View Team</li>`
                : ''
            }
          </ul>
        </div>
  ...
  async resetFilters() {
    ...
    if (topicReset) {
      this.trackFeatureUsageEvent(AccessTypeNo.BrowseAllTopics);
    }
    if (this.PromptActivity != DefaultPromptActivity) {
      this.trackFeatureUsageEvent(AccessTypeNo.BrowseAllActivities);
    }
    if (this.PromptModel != DefaultPromptModel) {
      this.trackFeatureUsageEvent(AccessTypeNo.FilterAllPromptModels);
    }
    if (this.PromptSearch != '') {
      this.trackFeatureUsageEvent(AccessTypeNo.ClearPromptSearch);
    }
  ...
  async changePromptPageSize(e) {
    ...
    this.trackFeatureUsageEvent(AccessTypeNo.SwitchPromptsPageSize);
  ...
  async changePromptTopic(e) {
    this.PromptTopic = e.target.value;
     ...
     if (this.PromptTopic === DefaultPromptTopic) {
      this.trackFeatureUsageEvent(AccessTypeNo.BrowseAllTopics);
    } else {
      this.trackFeatureUsageEvent(AccessTypeNo.BrowseTopic);
    }
  ...
  async changePromptActivity(e) {
    ...
    if (this.PromptActivity === DefaultPromptActivity) {
      ...
      this.trackFeatureUsageEvent(AccessTypeNo.BrowseActivity);
    }
  ...
  changePromptSortBy(e) {
    ...
    this.trackFeatureUsageEvent(AccessTypeNo.SwitchPromptSortBy);
  ...
  async changePromptModel(e) {
    ...
    if (this.PromptModel === DefaultPromptModel) {
      ...
      this.trackFeatureUsageEvent(AccessTypeNo.FilterPromptModel);
    }
  ...
  async changePromptSearch(e) {
    ...
    if (this.PromptSearch === '') {
      ...
      this.trackFeatureUsageEvent(AccessTypeNo.PromptSearch);
    }
  ...
  insertLanguageToneWritingStyleContinueActions() {
    ...
    if (parent.querySelector(`#${wrapper.id}`)) {
      ...
      // Check if #prompt-wrapper is present
      const promptWrapper = parent.querySelector('#prompt-wrapper');

      if (promptWrapper) {
        // Try inserting #language-select-wrapper right after #prompt-wrapper
        try {
          // DOM doesn't provide an insertAfter() directly, so we insert before the next sibling
          if (promptWrapper.nextSibling) {
            parent.insertBefore(wrapper, promptWrapper.nextSibling);
          } else {
            // If there's no next sibling, prepend it
            parent.prepend(wrapper);
          }
        } catch (error) {
          console.warn(
            'Could not insert language-select-wrapper after prompt-wrapper; falling back to prepend:',
            error
          );
          parent.prepend(wrapper);
        }
      } else {
        // Fallback if #prompt-wrapper is not found at all
        parent.prepend(wrapper);
      }
    }
        ...
        <select id="toneSelect" class="${css('select')}">
          ...
          ${
            !this.Client.UserQuota.canUseCustomTone()
              ...
              : '<option value disabled>_________</option><option value="ADD_NEW">Add tone...</option>'
          }
        </select>
      </div>
        ...
        <select id="writingStyleSelect" class="${css('select')}">
          ...
          ${
            !this.Client.UserQuota.canUseCustomWritingStyle()
              ? '<option value disabled>_________</option><option value="UPGRADE">Upgrade for more</option>'
              : '<option value disabled>_________</option><option value="ADD_NEW">Add writing style...</option>'
          }
        </select>
      </div>
    </div>
      ...
      <select id="continueActionSelect" class="${css('continueActionSelect')}">
        ...
        ${
          !this.Client.UserQuota.canUsePowerContinue()
            ...
            : '<option value disabled>_________</option><option value="ADD_NEW">Add power continue...</option>'
        }
      </select>
    </div>
  `;
    ...
    // Prevent event bubbling to prompt textarea
  ...
  insertIncludeMyProfileInfo() {
    ...
    const composerContainer = textarea.closest(
      selectorConfig.ComposerContainer
    );

    // If there is no composer container, skip
    if (!composerContainer) {
      console.error('insertMyProfileInfo: No composer container found');
      return;
    }

    // Toolbox buttons container to insert the "include my profile info" checkbox
    const toolboxButtonsContainer = composerContainer.querySelector(
      selectorConfig.ToolboxButtonsContainer
    );

    // If there is no button, skip
    if (!toolboxButtonsContainer) {
      console.error('insertMyProfileInfo: No toolbox buttons container found');
      return;
    }

    let wrapper = document.querySelector('#includeMyProfileInfo');
    let wrapperClassName =
      'AIPRM__p-2 AIPRM__pb-0 AIPRM__flex AIPRM__items-center AIPRM__whitespace-nowrap AIPRM__justify-end AIPRM__-mt-4';

    // Create the wrapper if it doesn't exist
    if (!wrapper) {
      wrapper = document.createElement('div');
      wrapper.className = wrapperClassName;
      wrapper.id = 'includeMyProfileInfo';

      // Update styling of toolbox buttons container
      toolboxButtonsContainer.classList.add(
        'AIPRM__max-w-full',
        'AIPRM__flex-wrap',
        '[&>*:last-of-type]:AIPRM__ml-auto',
        'AIPRM__gap-y-8'
      );

      // Get the submit button container
      const submitButtons = toolboxButtonsContainer.querySelectorAll(
        selectorConfig.SubmitButtonContainer
      );

      if (!submitButtons) {
        console.error('insertMyProfileInfo: No submit button container found');
        return;
      }

      // Insert as first child in the submit button container
      const submitButton = submitButtons?.length
        ? submitButtons[submitButtons.length - 1]
        : null;

      if (!submitButton) {
        console.error('insertMyProfileInfo: No submit button found');
        return;
      }

      submitButton?.classList.add('AIPRM__items-center');

      submitButton?.childNodes?.[0]?.before(wrapper);
    }

    // Hide for not linked accounts
    if (!this.Client.User.IsLinked) {
      wrapper.classList.add('AIPRM__hidden');
      this.SelectedMyProfileInfoID = null;
    } else {
      wrapper.classList.remove('AIPRM__hidden');
    }

    // Insert the checkbox
    wrapper.innerHTML = /*html*/ `
      <input id="includeMyProfileInput" name="includeMyProfile" type="checkbox" class="AIPRM__mr-2 AIPRM__mt-2 dark:AIPRM__bg-gray-850" 
            ${this.IncludeMyProfileMessage ? 'checked' : ''} ${
              this.MyProfileInfos.length > 0 ? '' : 'disabled'
            } />

      <div id="includeMyProfileInfoSelectWrapper" class="AIPRM__group">
        <div class="AIPRM__-mt-3 AIPRM__text-sm AIPRM__whitespace-nowrap">
          <label for="includeMyProfileInput"
          title="Include provided &quot;My Profile&quot; info that you would like ChatGPT to know and remember about you and your preferences.">Include</label>

          <label for="includeMyProfileInfoSelect" class="AIPRM__font-medium"><a class="AIPRM__underline AIPRM__cursor-pointer" title="Manage My Profile Info" href="${AppAccountURL}#myprofileinfo" target="_blank">My Profile Info</a></label>
        </div>

        <select id="includeMyProfileInfoSelect" class="AIPRM__bg-gray-100 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-600 dark:group-hover:AIPRM__bg-gray-800 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white group-hover:AIPRM__bg-gray-200 focus:AIPRM__ring-0 AIPRM__max-w-60 AIPRM__min-w-36 disabled:AIPRM__opacity-70 ${
    ...
    // Add event listener to wrapper to prevent event bubbling to prompt textarea
    wrapper.addEventListener('click', (e) => e.stopPropagation());
  },
  ...
  insertVariablesInputWrapper() {
    ...
    if (variableWrapper) {
      ...
      variableWrapper.className =
        'AIPRM__gap-3 @lg/thread:AIPRM__max-w-3xl @md/thread:last:AIPRM__mb-6 AIPRM__mx-2 AIPRM__pt-2 AIPRM__stretch AIPRM__justify-between AIPRM__text-sm AIPRM__pb-2 AIPRM__mb-2 AIPRM__border-b AIPRM__border-black/10 AIPRM__hidden';
  ...
  changeTone(event) {
    ...
    if (event.target.value === 'UPGRADE') {
      ...
      if (this.Client.UserQuota.canAddCustomToneWritingStylePowerContinue()) {
        window.open(AppAccountURL + '#tone', '_blank');
      }
  ...
  changeWritingStyle(event) {
    ...
    if (event.target.value === 'UPGRADE') {
      ...
      if (this.Client.UserQuota.canAddCustomToneWritingStylePowerContinue()) {
        window.open(AppAccountURL + '#style', '_blank');
      }
  ...
  changeContinueAction(event) {
    ...
    if (event.target.value === 'UPGRADE') {
      ...
      if (this.Client.UserQuota.canAddCustomToneWritingStylePowerContinue()) {
        window.open(AppAccountURL + '#continue', '_blank');
      }
  ...
  submitContinueActionPrompt(prompt = '') {
    ...
    const value =
      textarea.tagName === 'TEXTAREA' ? textarea.value : textarea.innerText;

    // If the textarea is not empty and it's not "Continue writing please" - ask for confirmation
    if (
      value.trim() &&
      value.trim() !== 'Continue writing please' &&
      !confirm(
    ...
    // wait for the button to be enabled in 500ms loop (max 5 retries)
    let retries = 0;

    const interval = setInterval(() => {
      // if the button is not found, retry
      if (!button || button.disabled) {
        button = document.querySelector(selectorConfig.PromptSubmitButton);
        retries++;
      }

      // if the button is found or the max retries is reached, clear the interval
      if (button || retries >= 5) {
  ...
  // Reset the continue action select to its default state and clear selected prompt
  resetContinueActionSelect() {
    // Reset the last selected continue action prompt
    this.SelectedContinueActionPrompt = null;

    const continueActionSelect = document.querySelector(
      '#continueActionSelect'
    );

    if (!continueActionSelect) {
      return;
    }

    // Reset to the first option (index 0)
    continueActionSelect.selectedIndex = 0;
  },

  // Decrement the current page of the prompt templates section and re-render
  async prevPromptTemplatesPage() {
    this.trackFeatureUsageEvent(AccessTypeNo.ClickPromptsPrevPage);
  ...
  async nextPromptTemplatesPage() {
    this.trackFeatureUsageEvent(AccessTypeNo.ClickPromptsNextPage);
  ...
  exportCurrentChat() {
    ...
    new Exporter(this.Config).exportCurrentChat();
  },
  ...
  async voteThumbsUp(idx) {
    try {
      await this.Client.voteForPrompt(
        (await this.getCurrentPromptTemplates())[idx].ID,
        1,
        this.CurrentGizmo?.GizmoCode
      );
    } catch (error) {
  ...
  async copyPromptDeepLink(idx) {
    ...
    const promptLink =
      prompt.PromptTypeNo === PromptTypeNo.PUBLIC
        ...
        : `https://chat.openai.com/?${queryParamPromptID}=${prompt.ID}`;
  ...
  async selectPromptTemplate(template) {
    ...
    // Check if #prompt-wrapper already exists
    let wrapper = parent.querySelector('#prompt-wrapper');

    if (!wrapper) {
      // If not found, create a new wrapper
      wrapper = document.createElement('div');
      wrapper.id = 'prompt-wrapper';

      // Check if #language-select-wrapper exists
      const languageSelectWrapper = parent.querySelector(
        '#language-select-wrapper'
      );

      if (languageSelectWrapper) {
        // Attempt to place #prompt-wrapper before #language-select-wrapper
        try {
          parent.insertBefore(wrapper, languageSelectWrapper);
        } catch (error) {
          console.warn(
            'Could not insert #prompt-wrapper before #language-select-wrapper; falling back to prepend:',
            error
          );
          parent.prepend(wrapper);
        }
      } else {
        // Fallback if there's no #language-select-wrapper
        parent.prepend(wrapper);
      }
    }
    ...
    if (template) {
      ...
      wrapper.innerHTML = /*html*/ `
        <span class="${css`tag`}" title="${sanitizeInput(template.Teaser)}">
          ...
          <a title="Deselect prompt / New chat"
            class="AIPRM__cursor-pointer AIPRM__ml-3 AIPRM__align-middle AIPRM__inline-block AIPRM__rounded-md hover:AIPRM__bg-gray-100 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-gray-700 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" 
        ...
        ${
          template.AuthorName
            ? /*html*/ `
              <span class="AIPRM__text-xs">by ${`<span class="AIPRM__mx-1" title="Created by ${sanitizeInput(
                template.AuthorName
      ...
      if (template.PromptVariables && template.PromptVariables.length > 0) {
        ...
        template.PromptVariables.forEach((promptVariable) => {
          ...
          // Prevent event bubbling to prompt textarea
          variableElement.addEventListener('click', (e) => e.stopPropagation());
        });
      }

      this.updatePromptTextareaPlaceholder(textarea, template.PromptHint);
      ...
      this.updatePromptTextareaPlaceholder(textarea, '');
                ...
                <div class="AIPRM__bg-white dark:AIPRM__bg-gray-900 AIPRM__px-4 AIPRM__pt-5 AIPRM__pb-4 sm:AIPRM__p-6 sm:AIPRM__pb-4 AIPRM__overflow-y-auto">
                  ...
                  <div class="AIPRM__block AIPRM__mt-4">
                    <div class="AIPRM__flex AIPRM__justify-between AIPRM__mt-4">
                      ...
                      <div class="AIPRM__w-full"><label>Author URL</label>
                        <input disabled name="AuthorURL" type="url" title="Author URL associated with this prompt template"
  ...
  async viewPromptTemplateSource(idx) {
    this.trackFeatureUsageEvent(AccessTypeNo.ViewPromptTemplateSource);
  ...
  async handleListSelectionModalSubmit(promptToSave = undefined, e) {
    ...
    if (!shareWithTeam && !listID) {
      if (!this.isAdminMode() && !this.Client.UserQuota.canUseCustomList()) {
        return;
      }
    }
  ...
  // Show AIPRM Account modal (connect/disconnect AIPRM account, current AIPRM Plan level and ChatGPT account)
  showAccountModal() {
          ...
          <div class="AIPRM__flex AIPRM__items-center AIPRM__justify-center AIPRM__min-h-full">
            ...
            <div
              ...
              role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                ...
                <div class="AIPRM__border-b AIPRM__border-black/10 dark:AIPRM__border-gray-700 AIPRM__px-6 AIPRM__flex AIPRM__w-full AIPRM__flex-row AIPRM__items-center AIPRM__justify-between">
                ...
                <div class="AIPRM__bg-white dark:AIPRM__bg-gray-900 AIPRM__p-4 AIPRM__px-6 AIPRM__overflow-y-auto">
                  ...
                  <dl class="AIPRM__text-sm">
                    <div class="AIPRM__flex AIPRM__py-4 AIPRM__flex-col sm:AIPRM__flex-row AIPRM__border-b AIPRM__border-black/10 dark:AIPRM__border-gray-700">
                      ...
                      <dd class="AIPRM__w-full sm:AIPRM__w-1/2 AIPRM__py-2 AIPRM__justify-between">
                        ...
                        ${
                          this.Client.User.IsLinked
                            ...
                            : this.shouldShowOAuth2ConnectCTA()
                              ? /*html*/ `
                                <button class="AIPRM__inline-block AIPRM__mt-4 AIPRM__bg-white AIPRM__border AIPRM__border-green-500 hover:AIPRM__border-green-700 AIPRM__text-green-500 hover:AIPRM__text-green-700 AIPRM__py-2 AIPRM__px-3 AIPRM__rounded dark:AIPRM__bg-gray-900 dark:hover:AIPRM__text-green-400 dark:hover:AIPRM__border-green-400" onclick="AIPRM.oauth2Login()">
                                  Login
                                </button>
                              `
                              : /*html*/ `
                    ...
                    <div class="AIPRM__flex AIPRM__py-4 AIPRM__flex-col sm:AIPRM__flex-row AIPRM__border-b AIPRM__border-black/10 dark:AIPRM__border-gray-700">
                      <dt class="AIPRM__w-full sm:AIPRM__w-1/2 AIPRM__py-2 AIPRM__font-medium">ChatGPT Account</dt>
                      <dd class="AIPRM__w-full sm:AIPRM__w-1/2 AIPRM__py-2 AIPRM__justify-between">
                        <div>
                           ${sanitizeInput(getUserOpenAI()?.email || 'Unknown')}
                        </div>
                      </dd>
                    </div>
    ...
    // add event listener to close the modal on ESC
    // abort any previous listener to prevent duplicates
    if (this._accountModalEscAbort) {
      this._accountModalEscAbort.abort();
    }
    this._accountModalEscAbort = new AbortController();
    document.addEventListener(
      ...
      { signal: this._accountModalEscAbort.signal }
    );
  },

  // Open app to connect AIPRM account
  connectAccount() {
    this.trackConnectEvent(AccessTypeNo.ConnectWeb);
  ...
  /**
   * Refresh user status from server and check if already connected.
   * Detects App-side disconnects by comparing local state against the server.
   *
   * @returns {Promise<boolean>} true if already connected
   */
  async isAlreadyConnected() {
    const wasLinked = this.Client?.User?.IsLinked;

    try {
      await this.Client.checkUserStatus();
    } catch (error) {
      console.warn(
        '[AIPRM] could not refresh user status before connect',
        error
      );
    }

    if (wasLinked && !this.Client?.User?.IsLinked) {
      console.log(
        '[AIPRM] stale IsLinked state detected, refreshed from server'
      );
    }
    ...
    if (this.Client?.User?.IsLinked) {
      this.showNotification(
        NotificationSeverity.INFO,
        'Your ChatGPT account is already connected to AIPRM.',
        false
      );
      return true;
    }
  ...
  /**
   * Initiate OAuth2 flow via AIPRM APP
   */
  async oauth2Login() {
    if (this._oauth2InProgress) {
      return;
    }
    this._oauth2InProgress = true;

    if (await this.isAlreadyConnected()) {
      this._oauth2InProgress = false;
      return;
    }

    const connectOperatorERID = this.Client?.User?.OperatorERID || '';
    const connectOperatorPlanType = this.Client?.User?.OperatorPlanType || '';

    try {
      this.trackConnectEvent(AccessTypeNo.ConnectOAuth2);

      // Hide the modal while connection is in progress
      this.hideModal('AIPRM__accountModal');
      ...
      connectModalHelper.updateState(
        NotificationSeverity.INFO,
        variant?.ConnectingMessage ||
          'Connecting your ChatGPT account to AIPRM...'
      );

      try {
        const loginPromise = this.OAuth2.initiateLogin();

        connectModalHelper.updateState(
          NotificationSeverity.INFO,
          variant?.WaitingMessage ||
            'Waiting for your AIPRM account connection to complete...'
        );
        ...
        if (result.success && result.tokens) {
          try {
            // Store AIPRM tokens received from background.js exchange
            this.Client.storeTokens(result.tokens);

            // Ensure we have an OperatorERID for linkUser()
            if (!this.Client.User?.OperatorERID) {
              const userOpenAI = getUserOpenAI();
            ...
            // Link operator via authenticated POST (Level 4 — OperatorERID only in POST body)
            await this.Client.linkUser();

            // Refresh AIPRM user profile and quota
            await this.Client.checkUserStatus();

            // Poll until link status is confirmed
            if (!this.Client.User?.IsLinked) {
              connectModalHelper.updateState(
                NotificationSeverity.INFO,
                variant?.ConnectingMessage ||
            ...
            console.error('[AIPRM OAuth2] post-auth error:', error);

            this.Client.resetTokens();
          ...
          // Redirect to connect-success page — AIPRM reinitializes on return
          window.location.href = appendLangParam(
            `${OAuth2Config.ConnectSuccessURL}?type=chatgpt&success=${ReactionNo.RXN_AIPRM_OAUTH2_CHATGPT_ACCOUNT_CONNECTED}${variant?.ConnectParams ? `&${variant.ConnectParams}` : ''}&connectOperatorERID=${encodeURIComponent(connectOperatorERID)}&planType=${encodeURIComponent(connectOperatorPlanType)}`
          );
        } else {
          const rxn = result.rxn;
          const message =
            ReactionMessage[rxn] ||
          ...
          connectModalHelper.updateState(
            NotificationSeverity.WARNING,
            rxn ? `${message} (${rxn})` : message
          );
        }
      } catch (error) {
        console.error('[AIPRM OAuth2] connection error:', error);

        connectModalHelper.updateState(
          NotificationSeverity.WARNING,
          variant?.ErrorConnectionMessage ||
  ...
  async disconnectAccount() {
    ...
    try {
      ...
      if (this.Client.User?.IsLinked) {
        ...
        await this.pollUntilLinkStatus(false);
      }
    } catch (error) {
      this.showNotification(
        NotificationSeverity.ERROR,
    ...
    // Clear stored AIPRM tokens after disconnect
    this.Client.resetTokens();

    try {
      localStorage.setItem(lastDisconnectKey, new Date().toISOString());
    } catch (error) {
      console.error(
    ...
    // Redirect to disconnect-success page — AIPRM reinitializes on return
    window.location.href = appendLangParam(
      `${OAuth2Config.DisconnectSuccessURL}?type=chatgpt&success=${ReactionNo.RXN_AIPRM_CHATGPT_ACCOUNT_DISCONNECTED_ANY}`
    );
  },
  ...
  /**
   * Get suffix for model status
   * @param {Model?} model
   */
  getModelStatusSuffix(model) {
    if (!model) {
      return '';
    } else if (model?.StatusNo == ModelStatusNo.ACTIVE) {
      return '';
    } else if (model?.StatusNo == ModelStatusNo.UNSUPPORTED) {
      return ' (Unsupported by this platform)';
    } else {
      return ' (Deprecated)';
    }
  },
  ...
  /**
   * Modify the conversation messages to not show the prompt template source
   *
   * @param {(() => Promise<Response>)} fetchConversationMessages
   * @returns {Promise<Response>}
   */
  async modifyConversationMessages(fetchConversationMessages) {
    try {
      // Get the chat messages (raw fetch API call)
      ...
      // Intercept the fetch messages response (JSON)
      let conversation = await response.json();

      // No conversation chat messages
      if (!conversation?.mapping) {
        console.error('No chat messages found in conversation:', conversation);

        return new Promise((resolve) => {
          resolve({
            ...response,
            text: () => Promise.resolve(JSON.stringify(conversation)),
      ...
      const reactFiber = Object.keys(firstUserMessage).find((key) =>
        key?.includes(selectorConfig.ReactFiberPropertyKey)
      );

      const userMessages =
        firstUserMessage?.[reactFiber]?.return?.memoizedProps?.messages;
      ...
      userMessages?.forEach((message) => {
        ...
        if (!conversation.mapping[message.message.id]) {
          console.error(
            'No conversation message found for user message:',
            message
          );
          return;
        }
        ...
        //   original: conversation.mapping[message.message.id].message.content,
        //   replacement: message.message.content,
        // });

        conversation.mapping[message.message.id].message.content =
          message.message.content;
      });

      // Return new Promise with modified response
      return new Promise((resolve) => {
        resolve({
          ...response,
          text: () => Promise.resolve(JSON.stringify(conversation)),
      ...
      console.error('Error in modifyConversationMessages:', error);

      return fetchConversationMessages();
    }
  },

  // Fetch models config from backend API for the prompt panel
  async fetchModelsConfig() {
    const config = this.Config.getPromptPanelConfig();

    // Check remote config to see if fetching models config is disabled
    if (
      !this.Config.isPromptPanelEnabled() ||
      !config.EnableModelsConfig ||
      !this.Client.AccessToken
    ) {
      return;
    }

    const modelsEndpoint = config.ModelsConfigEndpoint;
    ...
    try {
      ...
      if (!response.ok) {
        this.showNotification(
          NotificationSeverity.ERROR,
          'Could not fetch models config from backend API - invalid response.'
        );
        return;
      }
      ...
      if (!modelsConfig) {
        this.showNotification(
          NotificationSeverity.ERROR,
          'Could not fetch models config from backend API - empty response.'
        );
        return;
      }

      this.ModelsConfig = modelsConfig?.categories
        ?.map((model) => ({
          ID: model.default_model,
          Title: model.human_category_name,
          Description: model.short_explainer,
        }))
        ?.sort((a, b) => a.Title.localeCompare(b.Title));
    } catch (error) {
      this.showNotification(
        NotificationSeverity.ERROR,
        'Could not fetch models config from backend API - unexpected error.'
      );
    }
  },
};
