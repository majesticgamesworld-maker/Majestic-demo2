...
import { getUserOpenAI, getOperatorPlanType } from './utils.js';
 ...
 * @property {(PromptID: string, UsageTypeNo?: UsageTypeNo | AccessTypeNo, GizmoCode?: string, TraceID?: string) => Promise<Response>} usePrompt
...
const AIPRMClient = {
  ...
  /**
   * Extends fetch request with X-AIPRM-Version header
   */
  fetchExtended(url, options) {
    if (!options) {
      options = {};
    }
    ...
    return (
      fetch('/api/auth/session')
        ...
        .then(async (res) => {
          ...
          let OperatorERID = res?.user?.id || getUserOpenAI()?.id;

          // If OperatorERID is still not available and path is empty (homepage), try to fetch it from /backend-anon/me
          if (!OperatorERID && window.location.pathname === '/') {
            try {
              const anonResponse = await fetch('/backend-anon/me');
              const anonData = await anonResponse.json();

              OperatorERID = anonData.id;
            } catch (error) {
          ...
          this.User = {
            ...
            OperatorPlanType:
              res?.account?.planType || getOperatorPlanType(OperatorERID),
          ...
          const isAnonOperator =
            typeof OperatorERID === 'string' &&
            OperatorERID.startsWith(AnonOperatorERIDPrefix);

          // No access token is available in the ChatGPT session API response for anonymous users
          if (!this.AccessToken && !isAnonOperator) {
  ...
  checkUserStatus() {
    ...
    return (
      this.fetchExtended(
        `${this.APIEndpoint}/Users/Status?OperatorERID=${this.User.OperatorERID}&SystemNo=${this.User.SystemNo}&UserFootprint=${this.User.UserFootprint}&DetectedLocale=${encodeURIComponent(detectedLocale)}`
      )
        .then(this.handleResponse)
        // set the user status
        .then((res) => {
          ...
          if (Object.prototype.hasOwnProperty.call(res, 'Reseller')) {
            this.User.Reseller = res.Reseller;
          }
  ...
  voteForPrompt(PromptID, Vote, GizmoCode = undefined) {
    ...
    return this.fetchExtended(`${this.APIEndpoint}/Vote/${PromptID}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  reportPrompt(PromptID, FeedbackTypeNo, FeedbackText, FeedbackContact) {
    return this.fetchExtended(
      `${this.APIEndpoint}/Prompts/${PromptID}/Feedback`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
   ...
   *
   * @param {string} PromptID
   * @param {UsageTypeNo|AccessTypeNo} UsageTypeNo
   * @param {string|undefined} GizmoCode
   * @param {string|undefined} TraceID
   */
  usePrompt(
    PromptID,
    UsageTypeNo = UsageTypeNo.CLICK,
    GizmoCode = undefined,
    TraceID = undefined
  ) {
    const body = {
      UsageTypeNo: UsageTypeNo,
      // Use provided TraceID for performance tracking, if available
      User: TraceID ? { ...this.User, UserFootprint: TraceID } : this.User,
    };
    ...
    return this.fetchExtended(`${this.APIEndpoint}/Prompts/${PromptID}/Use`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  augmentPrompt(
    ...
    return this.fetchExtended(`${this.APIEndpoint}/PromptAugment`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  voteForMessage(MessageID, VoteTypeNo) {
    return this.fetchExtended(`${this.APIEndpoint}/Vote/${MessageID}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        ...
        // API VoteRequest.Vote allows only 1 or -1, so dismiss is tracked as +1
  ...
  unlinkUser() {
    ...
    return this.fetchExtended(`${APIEndpoint}/Users/Disconnect`, {
      method: 'POST',
      body: JSON.stringify({
        User: this.User,
      }),
  ...
  createList(TypeNo, Comment = '', FirstItem = {}, ForTeamID = undefined) {
    ...
    return this.fetchExtended(`${APIEndpoint}/Lists`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  updateList(
    ...
    return this.fetchExtended(`${APIEndpoint}/List/${List.ID}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  addToList(ListID, PromptID) {
    return this.fetchExtended(`${APIEndpoint}/List/${ListID}/Items`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  createMyProfileInfo(myProfileInfo) {
    return this.fetchExtended(
      `${APIEndpoint}/User/AccountSubPrompts/MyProfileInfo`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  useGizmo(GizmoCode, GizmoVoteTypeNo, GizmoConfig = undefined) {
    ...
    return this.fetchExtended(`${this.APIEndpoint}/Gizmos/${GizmoCode}/Vote`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  submitNewGizmo(gizmoConfig) {
    ...
    return this.fetchExtended(`${this.APIEndpoint}/Gizmos`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
