...
const svg = function (name) {
  ...
  switch (name) {
    ...
    case 'Save':
      return '<svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" height="18" width="18"><path stroke-linecap="round" stroke-linejoin="round" d="M17 16v2a2 2 0 01-2 2H5a2 2 0 01-2-2v-7a2 2 0 012-2h2m3-4H9a2 2 0 00-2 2v7a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-1m-1 4l-3 3m0 0l-3-3m3 3V3"></path></svg>';
    case 'Cross':
    ...
    case 'ChevronDown':
      return '<svg fill="none" stroke="currentColor" height="4rem" width="4rem" aria-hidden="true" stroke-width="1" class="AIPRM__h-4 AIPRM__w-4" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="m19.5 8.25-7.5 7.5-7.5-7.5" stroke-linecap="round" stroke-linejoin="round"></path></svg>';
    case 'ChevronUp':
      return '<svg fill="none" stroke="currentColor" height="4rem" width="4rem" aria-hidden="true" stroke-width="1" class="AIPRM__h-4 AIPRM__w-4" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="m4.5 15.75 7.5-7.5 7.5 7.5" stroke-linecap="round" stroke-linejoin="round"></path></svg>';
  }
};
...
const css = function (name) {
  ...
  switch (name) {
    ...
    case 'languageSelectWrapper':
      return 'AIPRM__flex AIPRM__gap-3 @lg/thread:AIPRM__max-w-3xl @md/thread:last:AIPRM__mb-6 AIPRM__ml-2 @md/thread:AIPRM__ml-0 AIPRM__pt-2 AIPRM__stretch AIPRM__justify-between AIPRM__text-sm AIPRM__items-end AIPRM__pb-2 AIPRM__mb-2 AIPRM__border-b AIPRM__border-black/10 AIPRM__flex-col @md/thread:AIPRM__flex-row dark:AIPRM__border-gray-700';
    case 'select':
...
/**
 * Parse and retrieve clientBootstrap data from script tag fallback.
 *
 * @returns {Object|null} The parsed clientBootstrap data or null if not found.
 */
const getClientBootstrapFromScript = function () {
  try {
    const scriptElement = document.querySelector('script#client-bootstrap');

    if (scriptElement?.textContent) {
      return JSON.parse(scriptElement.textContent);
    }
  } catch (e) {
    console.warn('AIPRM: Failed to parse client-bootstrap script tag', e);
  }
...
/**
 * Retrieve the OpenAI user from various global window objects, if available.
 *
 * @returns {Object} The user object from window or null if not found.
 */
const getUserOpenAI = function () {
  // Check possible places where user info might exist
  const user =
    window?.__reactRouterContext?.state?.loaderData?.root?.clientBootstrap
      ?.user ||
    window?.__remixContext?.state?.loaderData?.root?.clientBootstrap?.user ||
    window?.CLIENT_BOOTSTRAP?.user ||
    window?.__NEXT_DATA__?.props?.pageProps?.user ||
    getClientBootstrapFromScript()?.user ||
    null;

  if (!user) {
    console.warn('AIPRM: Unable to retrieve OpenAI user from any source');
  }

  return user;
};

// Retrieve operator plan type, if available
const getOperatorPlanType = function (operatorERID) {
  if (operatorERID?.startsWith(AnonOperatorERIDPrefix)) {
    return 'anon';
  }
  ...
  const planType =
    ...
    window?.CLIENT_BOOTSTRAP?.session?.account?.planType ||
    getClientBootstrapFromScript()?.session?.account?.planType ||
    null;

  if (!planType) {
    console.warn(
      'AIPRM: Unable to retrieve operator plan type from any source'
    );
  }
...
/**
 * Retrieve operator locale from clientBootstrap, if available.
 *
 * @returns {string}
 */
const getOperatorLocale = function () {
  return (
    window?.__reactRouterContext?.state?.loaderData?.root?.clientBootstrap
      ?.locale ||
    window?.CLIENT_BOOTSTRAP?.locale ||
    getClientBootstrapFromScript()?.locale ||
    ''
  );
};
...
// Lazily create and return a shared container for notifications and messages
const getNotificationContainer = function () {
  const containerID = 'AIPRM-NotificationContainer';
  ...
  if (!container) {
    ...
    container.className =
      'AIPRM__fixed AIPRM__flex AIPRM__flex-col AIPRM__items-center AIPRM__gap-2 AIPRM__w-full AIPRM__top-2 AIPRM__px-2 AIPRM__z-[100] AIPRM__pointer-events-none';
...
const DETECTED_LOCALE_KEY = 'AIPRM_detectedLocale';

/**
 * Detect the most region-specific user locale available.
 * Always re-evaluates primary sources (operator locale, navigator) so that
 * language changes take effect without requiring a localStorage clear.
 * localStorage is only consulted as a fallback when primary sources are empty.
 *
 * @param {string} fallback - value when no locale can be detected (default: '')
 * @returns {string}
 */
const detectLocale = function (fallback = '') {
  // ChatGPT falls back to 'en' for languages it doesn't support (e.g.
  // Filipino), which masks the user's real preference.  When the operator
  // locale is English, prefer navigator.language so the server receives
  // the user's actual top-priority locale.
  const operatorLocale = getOperatorLocale();
  const operatorIsEnglish =
    operatorLocale && operatorLocale.split('-')[0].toLowerCase() === 'en';

  let locale;
  if (operatorLocale && !operatorIsEnglish) {
    locale = operatorLocale;
  } else {
    locale = navigator.language || operatorLocale || '';
  }

  // If navigator.language has no region subtag (e.g. 'de' instead of 'de-AT'),
  // look through navigator.languages for a region-specific variant of the same language
  if (locale && !locale.includes('-') && navigator.languages?.length) {
    const base = locale.toLowerCase();
    const regionSpecific = navigator.languages.find(
      (lang) => lang.includes('-') && lang.toLowerCase().startsWith(base + '-')
    );
    if (regionSpecific) {
      locale = regionSpecific;
    }
  }

  if (!locale) {
    try {
      locale = localStorage.getItem(DETECTED_LOCALE_KEY) || '';
    } catch {
  ...
  if (!locale) {
    locale = document.documentElement.dataset.aiprmChromeUiLang || fallback;
  }

  if (locale) {
    try {
      localStorage.setItem(DETECTED_LOCALE_KEY, locale);
    } catch {
      // localStorage unavailable
    }
  }
...
/**
 * Find the first 2-letter language code from the user's preferred languages
 * that exists in the given set of supported languages.
 * Checks operator locale first, then walks navigator.languages.
 *
 * @param {Set<string>|string[]} supportedLangs - supported 2-letter codes
 * @param {string} fallback - fallback code when no match found (default: 'en')
 * @returns {string}
 */
const getPreferredSupportedLang = function (supportedLangs, fallback = 'en') {
  const supported =
    supportedLangs instanceof Set
      ? supportedLangs
      : new Set(supportedLangs);

  const candidates = [];

  // ChatGPT falls back to 'en' for languages it doesn't support (e.g.
  // Filipino), which masks the user's real preference from navigator.languages.
  // Only give operator locale top priority when it is NOT English.
  const operatorLocale = getOperatorLocale();
  if (operatorLocale && operatorLocale.split('-')[0].toLowerCase() !== 'en') {
    candidates.push(operatorLocale);
  }
...
/**
 * Replace default legal URLs in HTML content with reseller-specific ones.
 *
 * @param {string} html - HTML content potentially containing default legal URLs
 * @param {import('./config.js').Config} config - Remote config instance
 * @param {string|undefined} reseller - User's reseller value from API
 * @returns {string} HTML with replaced legal URLs
 */
const replaceLegalURLs = (html, config, reseller) => {
  const legalURLs = config?.getLegalURLsConfig?.();
  if (!legalURLs || !html) return html || '';

  const defaults = legalURLs.Default;
  if (!defaults) return html;

  const resellerURLs = (reseller && legalURLs[reseller]) || defaults;

  let result = html;
  if (defaults.TermsURL && resellerURLs.TermsURL) {
    result = result.replaceAll(defaults.TermsURL, resellerURLs.TermsURL);
  }
  if (defaults.PrivacyURL && resellerURLs.PrivacyURL) {
    result = result.replaceAll(defaults.PrivacyURL, resellerURLs.PrivacyURL);
  }
