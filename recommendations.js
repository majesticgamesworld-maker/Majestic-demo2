...
function updateMajesticBuddyInsight(mode) {
  ...
  var modes = {
    ...
    age: {
      ...
      text: 'Our WhatsApp team will confirm whether ' + product.name + ' is right for ' + facts.ageRange + ', including safety, maturity level and who should supervise play.',
      focus: 'age suitability and safety'
    },
    players: {
      title: 'Check if the group size works.',
      text: 'Best listed player fit is ' + facts.players + '. Ask us to confirm whether it will still feel good for your exact group size and table setup.',
      focus: 'player count and table setup'
    },
    delivery: {
      title: 'Check stock and delivery timing.',
      text: 'We can confirm live stock, edition details, Nairobi delivery timing, Kenya-wide courier options and the final delivery fee before you pay.',
      focus: 'stock, edition and delivery timing'
    },
    gift: {
      title: 'Check if it is gift-ready.',
      text: 'Our team will confirm whether this is a good gift for ' + facts.bestFor.slice(0, 2).join(' or ') + ', and suggest a better option if the recipient needs something different.',
  ...
  if (ctaEl) {
    ...
    ctaEl.textContent = mode === 'overall' ? 'Confirm fit on WhatsApp' : 'Ask us to check this';
  }
...
function getPdpRatingStars() {
  return '<span class="pdp-star-icons" aria-hidden="true">' +
    getPdpIcon('star') +
    getPdpIcon('star') +
    getPdpIcon('star') +
    getPdpIcon('star') +
...
function getProductLearningValue(product, facts) {
  var text = getProductSearchText(product).toLowerCase();
  if (product.adult || /adult|drinking|intimate|bondage|naughty/.test(text)) {
    return 'The value is social: it can open conversation, shared laughter and playful connection for adults who choose the right setting.';
  }
  if (/stem|science|robot|rocket|construction|lego|learning|educational/.test(text)) {
    return 'It supports hands-on curiosity, following steps, patience and problem solving while play stays active.';
  }
  if (/puzzle|jigsaw|tangram|memory|spot it|dobble/.test(text)) {
    return 'It encourages observation, patience, pattern spotting and the satisfying focus of finishing a challenge.';
  }
  if (/chess|strategy|catan|azul|scrabble|rummikub|qwirkle|sequence/.test(text)) {
    return 'It invites planning, turn taking, pattern spotting and decision making that sharpen with repeat plays.';
  }
  if (product.ageGroup === 'kids' || /kid|child|junior|school/.test(text)) {
    return 'It helps younger players practise turns, confidence, listening and screen-free time with others.';
  }
  return 'It builds social play through turns, attention, friendly decisions and shared time away from screens.';
}

function getProductFunValue(product, facts, profile) {
  var bestFor = facts.bestFor.slice(0, 2).join(' and ');
  return profile.subtitle + ' That makes ' + product.name + ' a strong fit for ' + bestFor + ' when the group wants ' + profile.vibe.toLowerCase() + '.';
}

function renderProductFaqs(product, facts, profile, metadataStatus) {
  var faqs = [
    ['Who is ' + product.name + ' suited to?', 'Start with ' + facts.ageRange + ', ' + facts.players + ' and ' + facts.bestFor.slice(0, 3).join(', ') + '. Check the exact edition if age guidance on the box matters for your order.'],
    ['How does ' + product.name + ' play?', 'Expect a ' + facts.playtime + ' session with a ' + profile.difficulty.toLowerCase() + ' entry point. Use the How to Play tab above for the step-by-step overview.'],
    ['What occasion does it fit?', facts.occasion + '. It is also worth comparing related games below if you want a different pace or group size.'],
    ['Can I confirm box contents and delivery before buying?', metadataStatus.exact
      ? 'Yes. This title has researched catalogue details, and WhatsApp can still confirm the current edition, stock and delivery fee.'
      : 'Yes. This page uses catalogue guidance, so WhatsApp is the best place to confirm the current edition, box contents, stock and delivery fee before paying.']
  ];
  return '<div class="pdp-faq-list">' + faqs.map(function(item) {
    return '<details><summary>' + escHtml(item[0]) + '</summary><p>' + escHtml(item[1]) + '</p></details>';
  }).join('') + '</div>';
}

function renderProductSeoLinks(product, cat) {
  var categoryLabel = cat ? cat.label : 'similar games';
  var categoryLinks = [
    ['category', product.cat, 'Browse ' + categoryLabel],
    ['category', 'family-games', 'Family games hub'],
    ['category', 'party-games', 'Party games hub'],
    ['category', 'kids-games', 'Kids games hub'],
    ['category', 'couples-games', 'Couples games hub'],
    ['gift-picks', '', 'Gift picks']
  ].filter(function(item, index, items) {
    return items.findIndex(function(candidate) { return candidate[0] + ':' + candidate[1] === item[0] + ':' + item[1]; }) === index;
  }).map(function(item) {
    var page = item[0];
    var id = item[1];
    var click = id
      ? 'return handleRouteClick(event,\'' + page + '\',\'' + id + '\')'
      : 'return handleRouteClick(event,\'' + page + '\')';
    return '<a href="' + getRouteUrl(page, id) + '" onclick="' + click + '">' + escHtml(item[2]) + '</a>';
  }).join('');
  var guideLinks = [
    ['/blog/family-friendly-party-games-everyone-can-play/', 'Family-friendly party guide'],
    ['/blog/card-games-perfect-for-parties-and-family-nights/', 'Party and family card guide'],
    ['/blog/adult-party-games-vs-family-party-games/', 'Adult vs family party guide'],
    ['/blog/', 'Game guides and ideas']
  ].map(function(item) {
    return '<a href="' + item[0] + '">' + escHtml(item[1]) + '</a>';
  }).join('');
  return '<div class="pdp-seo-links"><div><h3>Related game hubs</h3><nav aria-label="Related game categories">' + categoryLinks + '</nav></div><div><h3>Guides for choosing well</h3><nav aria-label="Related game guides">' + guideLinks + '</nav></div></div>';
}
...
function getPdpComplementaryCategories(product) {
  var map = {
    'couples-games': ['party-games', 'family-games', 'drinking-games'],
    'drinking-games': ['party-games', 'family-games', 'couples-games'],
    'party-games': ['family-games', 'drinking-games', 'board-games'],
    'card-games': ['party-games', 'family-games', 'board-games'],
    'board-games': ['family-games', 'puzzles', 'party-games'],
    'family-games': ['board-games', 'kids-games', 'party-games'],
  ...
  return map[product.cat] || ['family-games', 'party-games', 'board-games'];
}
...
function renderProductPage(productId) {
  ...
  var gameplaySummary = howItPlaysPlain.slice(0, 2).join(' ');
  var learningValue = getProductLearningValue(product, facts);
    ...
    content.innerHTML =
      '<section class="pdp-ref-hero pdp-full-span">' +
        ...
        '<aside class="pdp-ref-buy product-detail-panel" aria-label="Product purchase options">' +
          ...
          '<div class="pdp-ref-rating" aria-label="4.8 out of 5 stars">' + getPdpRatingStars() + '<b>4.8 (128 reviews)</b></div>' +
      ...
      '<section class="pdp-seo-depth pdp-full-span" aria-labelledby="pdp-seo-depth-title">' +
        ...
        '<div class="pdp-seo-depth-grid">' +
          '<article><h3>Gameplay</h3><p>' + escHtml(gameplaySummary) + '</p></article>' +
          '<article><h3>Age suitability</h3><p>Recommended starting point: ' + escHtml(facts.ageRange) + '. Match the box guidance to the players before ordering when safety or difficulty matters.</p></article>' +
          '<article><h3>Occasion fit</h3><p>' + escHtml(facts.occasion) + '. It suits ' + escHtml(facts.bestFor.slice(0, 3).join(', ')) + '.</p></article>' +
          '<article><h3>Educational value</h3><p>' + escHtml(learningValue) + '</p></article>' +
          '<article><h3>Why it is fun</h3><p>' + escHtml(funValue) + '</p></article>' +
          '<article><h3>Compare before you choose</h3><p>Use related games, category hubs and guides below to compare pace, group fit and gifting options in Kenya.</p></article>' +
        '</div>' +
        seoLinks +
      '</section>' +
...
function getComparableImageSrc(src) {
  return String(src || '').split('?')[0].split('#')[0].replace(/^.*\/images\//, 'images/').replace(/^.*\/product-images\//, 'product-images/');
}

function findImageZoomIndex(src, gallery) {
  var target = getComparableImageSrc(src);
  var list = gallery || imageZoomGallery;
  for (var i = 0; i < list.length; i++) {
    if (getComparableImageSrc(list[i]) === target || String(src || '').indexOf(list[i]) !== -1) return i;
  }
  return 0;
}

function renderImageZoomThumbs() {
  var thumbs = document.getElementById('image-zoom-thumbs');
  var prev = document.getElementById('image-zoom-prev');
  var next = document.getElementById('image-zoom-next');
  var hasGallery = imageZoomGallery.length > 1;
  if (prev) prev.style.display = hasGallery ? 'grid' : 'none';
  if (next) next.style.display = hasGallery ? 'grid' : 'none';
  if (!thumbs) return;
  thumbs.style.display = hasGallery ? 'flex' : 'none';
  if (!hasGallery) {
    thumbs.innerHTML = '';
    return;
  }
...
function setImageZoomImage(index) {
  if (!imageZoomGallery.length) return;
  if (index < 0) index = imageZoomGallery.length - 1;
  if (index >= imageZoomGallery.length) index = 0;
  imageZoomIndex = index;
  imageZoomScale = 1;
  var img = document.getElementById('image-zoom-img');
  var title = document.getElementById('image-zoom-title');
  var product = getProductById(currentProduct);
  var baseAlt = product ? product.name + ' product image' : (img && img.alt) || 'Product image';
  if (img) {
    img.src = imageZoomGallery[index];
    img.alt = baseAlt + ' ' + (index + 1) + ' of ' + imageZoomGallery.length;
  }
  if (title) title.textContent = baseAlt + ' ' + (index + 1) + ' of ' + imageZoomGallery.length;
  renderImageZoomThumbs();
  applyImageZoomScale();
}

function changeImageZoomImage(delta) {
  if (imageZoomGallery.length < 2) return;
  setImageZoomImage(imageZoomIndex + delta);
}

function openImageZoom(src, alt, gallery, startIndex) {
  var modal = document.getElementById('image-zoom-modal');
  var img = document.getElementById('image-zoom-img');
  var title = document.getElementById('image-zoom-title');
  if (!modal || !img) return;

  imageZoomGallery = (gallery && gallery.length ? gallery.slice() : [src]).filter(Boolean);
  imageZoomIndex = typeof startIndex === 'number' ? startIndex : findImageZoomIndex(src, imageZoomGallery);
  imageZoomScale = 1;
  img.src = imageZoomGallery[imageZoomIndex] || src || '';
  img.alt = alt || 'Product image';
  if (title) title.textContent = (alt || 'Product image') + (imageZoomGallery.length > 1 ? ' ' + (imageZoomIndex + 1) + ' of ' + imageZoomGallery.length : '');
  renderImageZoomThumbs();
  applyImageZoomScale();
  modal.classList.add('open');
  modal.setAttribute('aria-hidden', 'false');
  document.body.style.overflow = 'hidden';
  if (imageZoomFocusTrap) imageZoomFocusTrap(false);
  imageZoomFocusTrap = focusTrap(modal.querySelector('.image-zoom-shell'));
}
...
function closeImageZoom(event) {
  ...
  if (imageZoomFocusTrap) imageZoomFocusTrap();
  imageZoomFocusTrap = null;
  imageZoomScale = 1;
  imageZoomGallery = [];
  imageZoomIndex = 0;
}
...
function renderCartPage() {
  ...
  if (cart.length === 0) {
    content.innerHTML = '<div class="empty-state"><div class="empty-state-icon">Cart</div><h3 class="empty-state-title">Your cart is empty</h3><p class="empty-state-sub">Add some games to get started!</p><button class="btn-coral" onclick="navigate(\'shop\')">Browse Games</button></div>';
    return;
  }
...
function getMetaIconSvg(type) {
  var icons = {
    category: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4" y="4" width="7" height="7" rx="1.5"/><rect x="13" y="4" width="7" height="7" rx="1.5"/><rect x="4" y="13" width="7" height="7" rx="1.5"/><rect x="13" y="13" width="7" height="7" rx="1.5"/></svg>',
    players: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 21v-2a4 4 0 0 0-8 0v2"/><circle cx="12" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M2 21v-2a4 4 0 0 1 3-3.87"/></svg>',
    age: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 21h8"/><path d="M12 17v4"/><path d="M7 17h10v-6H7v6z"/><path d="M9 11V7"/><path d="M15 11V7"/><path d="M9 7c1-1 1-2 0-3"/><path d="M15 7c1-1 1-2 0-3"/></svg>'
  };
...
function openQuickView(productId) {
  ...
  var meta = '';
  if (cat) meta += '<span class="modal-meta-item">' + getMetaIconSvg('category') + escHtml(cat.label) + '</span>';
  if (product.players) meta += '<span class="modal-meta-item">' + getMetaIconSvg('players') + escHtml(product.players) + '</span>';
  if (product.age) meta += '<span class="modal-meta-item">' + getMetaIconSvg('age') + escHtml(product.age) + '</span>';
  ...
  document.body.style.overflow = 'hidden';
  if (quickViewFocusTrap) quickViewFocusTrap(false);
  quickViewFocusTrap = focusTrap(modalContent);
}
...
function closeQuickView(event) {
  ...
  if (quickViewFocusTrap) quickViewFocusTrap();
  quickViewFocusTrap = null;
}
...
function focusTrap(container) {
  ...
  function onKeydown(event) {
    if (event.key === 'Tab') trapFocusIn(container, event);
  }
